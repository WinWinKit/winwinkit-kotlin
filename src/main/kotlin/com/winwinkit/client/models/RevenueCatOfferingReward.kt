/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package com.winwinkit.client.models


import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param key The key of the reward
 * @param offeringId The offering ID of the reward
 * @param offeringKey The offering key of the reward
 * @param name The name of the reward
 * @param description The description of the reward
 * @param metadata The metadata of the reward
 * @param createdAt The created at of the reward
 * @param updatedAt The updated at of the reward
 */


data class RevenueCatOfferingReward (

    /* The key of the reward */
    @Json(name = "key")
    val key: kotlin.String,

    /* The offering ID of the reward */
    @Json(name = "offering_id")
    val offeringId: kotlin.String,

    /* The offering key of the reward */
    @Json(name = "offering_key")
    val offeringKey: kotlin.String,

    /* The name of the reward */
    @Json(name = "name")
    val name: kotlin.String,

    /* The description of the reward */
    @Json(name = "description")
    val description: kotlin.String?,

    /* The metadata of the reward */
    @Json(name = "metadata")
    val metadata: kotlin.Any?,

    /* The created at of the reward */
    @Json(name = "created_at")
    val createdAt: java.time.OffsetDateTime,

    /* The updated at of the reward */
    @Json(name = "updated_at")
    val updatedAt: java.time.OffsetDateTime?

) {


}

